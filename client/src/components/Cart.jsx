...
const Cart = () => {
  ...
  return (
    ...
    >
      <div className="flex w-full items-center justify-between py-4 pb-2 px-6">
        <motion.i
          {...buttonClick}
      ...
      <div className="flex flex-1 flex-col items-start justify-start rounded-t-3xl bg-zinc-900 h-full py-6 gap-3 relative">
        {cart && cart?.length > 0 ? (
          ...
          <>
            <h1 className="text-3xl text-primary font-bold flex flex-wrap place-content-center px-8">
              Cart is Empty
            </h1>
          </>
        )}
...
export const CartItemCard = ({ index, data }) => {
  ...
  const decrementCart = (product_id) => {
    ...
    increaseItemQuantity(user?.user_id, product_id, "decrement").then(
      (data) => {
  ...
  const incrementCart = (product_id) => {
    dispatch(alertSuccess("Updated the cart item"));
    increaseItemQuantity(user?.user_id, product_id, "increment").then(
      (data) => {
        getAllCartItems(user?.user_id).then((items) => {
          dispatch(setCartItems(items));
          dispatch(alertNULL());
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-start gap-1 w-full">
        ...
        <p className="text-lg font-semibold text-red-400 ml-auto flex items-center justify-center gap-1">
