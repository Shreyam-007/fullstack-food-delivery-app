...
const DBNewItem = () => {
    ...
    uploadTask.on(
      ...
      () => {
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          ...
          setIsLoading(false);
          setProgress(null);
          dispatch(alertSuccess("Image uploaded to the cloud successfully"));
          setTimeout(() => {
            dispatch(alertNULL());
          }, 3000);
        });
  ...
  const deleteImageFromFirebase = () => {
    ...
    deleteObject(deleteRef).then(() => {
      ...
      setIsLoading(false);
      dispatch(alertSuccess("Image removed from the cloud successfully"));
      setTimeout(() => {
        dispatch(alertNULL());
      }, 3000);
    });
  ...
  return (
    <div className="flex items-center justify-center flex-col pt-6 px-24 w-full">
      <div className="border border-gray-300 rounded-md p-4 w-full flex flex-col items-center justify-center gap-4">
        ...
        <div className="flex w-full items-center justify-around gap-3 flex-wrap">
          {statuses &&
            statuses?.map((data) => (
              <p
                key={data.id}
        ...
        <motion.button
          ...
          {...buttonClick}
          className="w-9/12 bg-red-500 text-white px-4 py-2 rounded-md hover:bg-red-600 cursor-pointer font-semibold"
        >
          {" "}
          Add Item
        </motion.button>
